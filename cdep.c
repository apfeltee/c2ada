...
static void add_suppress(path) char* path;
{
...
static int suppress(path) char* path;
{
    int i, len;
...
static char* file_extension(path) char* path;
{
    char* last_dot = NULL;
...
static void strcpy_until(s1, s2, stop) char *s1, *s2, *stop;
{
    while(*s2 && s2 != stop)
    {
        *s1++ = *s2++;
...
static void strcpy_root(p, path) char *p, *path;
{
    char* last;
...
static char* output_file(fname) char* fname;
{
...
static void get_depends(file, f) int file;
FILE* f;
{
    char* fname;
    char* lhs;
...
static void dump_depends(f) FILE* f;
{
    int i;
    for(i = 0; i < MAX_PARENTS; i++)
    {
...
static int patch_copy(mf, tf, tfname) FILE *mf, *tf;
char* tfname;
{
    char buf[2048];
    int len;
...
static void patch_mf(makefile) char* makefile;
{
    FILE *mf, *tf;
    int tfd;
    char tfname[16];
...
static void set_parent(pos) file_pos_t pos;
{
    cur_parent = FILE_ORD(pos);
    parents[cur_parent] = 1;
}

static void set_depend(pos) file_pos_t pos;
{
    int i;
...
static int scan_directive(c) int c;
{
    char fname[256];
    int i;
