...
static stmt_pt new_scoped_stmt_Expr(scope_id_t scope, node_pt e)
{
    set_current_scope(scope);
    return new_stmt_Expr(e);
}

static typeinfo_pt type_univ_int(void)
...
static typeinfo_pt type_natural(void)
...
static typeinfo_pt type_ptrdiff(void)
...
static typeinfo_pt type_size_t_(void)
...
static node_pt zero(void)
{
    return new_node(_Int_Number, 0, 10);
}

static node_pt one(void)
{
    return new_node(_Int_Number, 1, 10);
}

static node_pt two(void)
{
    return new_node(_Int_Number, 2, 10);
}

static node_pt null(typeinfo_pt t)
{
...
new_tmpvar_node(typeinfo_pt type, ctxt_pt ctxt, file_pos_t pos, node_pt value, bool emit_initializer)
{
    ...
    if(value)
    {
        ...
        {
            append_pre(ctxt, new_scoped_stmt_Expr(
                             ctxt_scope(ctxt),
                             new_pos_node(value->node_def, _Assign, result, value)));
        }
    }
    return result;
}
...
static bool has_type_univ_int(node_pt e)
{
    ...
    if(e->node_kind == _Macro_ID)
    {
        ...
        return m->const_value.eval_result_kind == eval_int && !m->const_value.explicit_type;
    }
...
static stmt_pt combine_stmt_ctxt(stmt_pt s, ctxt_pt ctxt, file_pos_t pos, scope_id_t scope)
/*
 ...
 */
{
    ...
    if(added_decls(ctxt))
    {
        ...
        return new_stmt_Compound(pos, decls(ctxt, 0), s2);
    }
    else
    {
...
static stmt_pt fix_stmt_itself(stmt_pt stmt)
{
    ctxt_pt ctxt;
    stmt_pt s1;
    stmt_pt result;
...
void fix_func_body(symbol_t* func)
{
    ...
    if(!func->has_return && is_function(func))
    {
         ...
         */
        stmt_pt ret_stmt
        = new_stmt_Return(body->stmt_def, zero_of_type(func->sym_type->type_next));
        assert(body->stmt_kind == _Compound);
        body->stmt.compound.stmts = concat_stmts(body->stmt.compound.stmts, ret_stmt);
        warning_at(body->stmt_def, "function %s has no return statement; dummy generated",
                   func->sym_ada_name);
    }
    func->sym_value.body = body;
}

static typeinfo_pt integral_promotion(typeinfo_pt type)
 ...
 */
{
    ...
    {
        return (SIZEOF_LONG > SIZEOF_INT) ? type_long() : type_unsigned_long();
    }
...
static node_pt unchecked_conversion(node_pt e, typeinfo_pt to_type)
{
    ...
    return new_pos_node(pos, _Func_Call, new_pos_node(pos, _Sym, conversion), e);
}
...
static node_pt type_cast(node_pt e, typeinfo_pt to_type)
{
    ...
    else if(from_type->type_kind == array_of && to_type->type_kind == pointer_to)
    {
        node_pt first = new_pos_node(pos, _Array_Index, e, zero());
        node_pt fixed_first = fix_expr(first, 0, 0);
        return promote(new_pos_node(pos, _Addrof, fixed_first), to_type, 0);
    }
    else if(from_type->type_kind == pointer_to && to_type->type_kind != pointer_to)
    {
    ...
    {
        node_pt result
        = new_pos_node(pos, _Type_Cast, new_pos_node(pos, _Type, to_type), e);
...
static node_pt promote(node_pt e, typeinfo_pt to_type, ctxt_pt ctxt)
{
    ...
    else if(has_type_univ_int(e) && to_type->type_kind == int_type
    ...
    else if(equal_types(from_type, to_type))
    {
        ...
        else if(from_type->type_kind == pointer_to && from_type->type_next->type_kind == function_type)
        {
            return e;
        }
        else
...
static node_pt int_if_char(node_pt e)
{
...
static node_pt promote_integer(node_pt e)
{
    return promote(e, integral_promotion(type_of(e)), 0);
}

static typeinfo_pt type_of(node_pt expr)
{
    ...
    switch(expr->node_kind)
    {
        ...
        case _Arrow_Selected:
             ...
             */
            {
                ...
                {
                    sym = find_field(expr->node.binary.l, field,
        ...
        case _Cond:
        {
            ...
            else if(t1->type_kind == int_type || t1->type_kind == float_type)
            {
                ...
                result = common_type(expr->node.cond.tru, expr->node.cond.fals);
            }
            ...
            else
            {
                ...
                error(file_name(expr->node_def), line_number(expr->node_def),
...
static typeinfo_pt ada_type_of(node_pt e)
{
...
/* converts e to or from bool, as appropriate */
{
    ...
    else if(is_bool)
    {
        ...
        {
            result = new_pos_node(e->node_def, _BoolTyp, e);
        }
    }
    else
    {
...
static node_pt fix_expr_Binop(node_pt e, ctxt_pt ctxt, usage_flags usage)
{
    ...
    if(!bool_subexprs && !has_pointer_type(el) && !has_pointer_type(er))
    {
...
static node_pt fix_expr_Binop_additive(node_pt e, ctxt_pt ctxt, usage_flags flags)
/* + and - can involve pointer arithmetic */
{
    ...
    {
        return new_pos_node(e->node_def, _Addrof,
...
static typeinfo_pt type_to_unsigned(typeinfo_pt type)
{
    typeinfo_pt to_type;
...
static typeinfo_pt type_to_builtin(typeinfo_pt type)
 ...
 */
{
    ...
    static tf tarray[]
    = { type_int,  type_unsigned,      type_short,         type_unsigned_short,
        type_long, type_unsigned_long, type_unsigned_char, type_signed_char };
    ...
    {
        btype = (*tarray[i])();
        if(equal_types(type, btype))
            return btype;
    }
    return 0;
}

static node_pt to_unsigned(node_pt e)
...
static bool ok_signing(node_pt e, typeinfo_pt type)
{
    ...
    switch(e->node_kind)
    {
        ...
        case _Macro_ID:
        {
            ...
            {
                return !val_must_be_unsigned(m->const_value.eval_result.ival, size);
            }
            else
            {
                return TRUE;
...
static node_pt fix_expr_Binop_bitwise(node_pt e, ctxt_pt ctxt, usage_flags usage)
{
    node_pt el = fix_expr(e->node.binary.l, ctxt, 0);
    node_pt er = fix_expr(e->node.binary.r, ctxt, 0);
    typeinfo_pt to_type;
...
static bool has_side_effects(node_pt e)
{
    switch(e->node_kind)
    {
        ...
        case _Cond:
            return has_side_effects(e->node.cond.boolval)
 ...
 */
{
    ...
    result = fix_expr(new_pos_node(e->node_def, _Assign, e1, e1_op_e2), ctxt, usage);
    return result;

} /* fix_expr_Binop_Assign */

static node_pt fix_expr_Simple_Binop(node_pt e, ctxt_pt ctxt, usage_flags usage)
/*
...
static node_pt fix_expr_Type_Cast(node_pt e, ctxt_pt ctxt, usage_flags usage)
{
    if(USAGE(Is_stmt))
    {
...
static node_pt fix_expr_Array_Index(node_pt e, ctxt_pt ctxt, usage_flags usage)
{
    node_pt el = fix_expr(e->node.binary.l, ctxt, 0);
    node_pt er = fix_expr(e->node.binary.r, ctxt, 0);
    typeinfo_pt eltype;
    ...
    if(el->node_kind == _Array_Index)
    {
        ...
        e->node.binary.r = new_pos_node(e->node_def, _List, el->node.binary.r, er);
        return e;
    }
    else if((eltype = type_of(el))->type_kind == pointer_to)
    {
        /* convert a[i] => *(a+i) */
        return fix_expr(new_pos_node(e->node_def, _Indirect,
...
static node_pt fix_expr_Comma(node_pt e, ctxt_pt ctxt, usage_flags usage)
...
static node_pt fix_expr_Select(node_pt e, ctxt_pt ctxt, usage_flags usage)
{
    node_pt el = fix_expr(e->node.binary.l, ctxt, 0);
    node_pt er = fix_expr(e->node.binary.r, ctxt, 0);
    symbol_t* field = 0;
    ...
    if(er->node_kind == _Ident)
    {
        ...
        if(!field)
        {
            fatal(__FILE__, __LINE__, "field '%s' not in type; file %s line %d",
                  er->node.id.name, file_name(e->node_def), line_number(e->node_def));
        }
    }
...
static node_pt to_natural(node_pt e)
{
    return new_pos_node(e->node_def, _Type_Cast,
                        new_pos_node(e->node_def, _Type, type_natural()), e);
}

static node_pt fix_expr_Shift(node_pt e, ctxt_pt ctxt, usage_flags usage)
/* _Shl or _Shr */
{
    node_pt el = fix_expr(e->node.binary.l, ctxt, 0);
    node_pt er = fix_expr(e->node.binary.r, ctxt, 0);
...
static node_pt fix_expr_Addrof(node_pt e, ctxt_pt ctxt, usage_flags usage)
{
...
static node_pt fix_expr_Unop(node_pt e, ctxt_pt ctxt, usage_flags usage)
...
static node_pt fix_expr_Ones_Complement(node_pt e, ctxt_pt ctxt, usage_flags usage)
{
    node_pt esub;
...
static node_pt fix_expr_Not(node_pt e, ctxt_pt ctxt, usage_flags usage)
{
...
static node_pt fix_expr_Crement(node_pt e, ctxt_pt ctxt, usage_flags usage)
...
static node_pt fix_expr_Assign(node_pt e, ctxt_pt ctxt, usage_flags usage)
{
    node_pt el = fix_expr(e->node.binary.l, ctxt, 0);
    node_pt er = fix_expr(e->node.binary.r, ctxt, 0);
    stmt_pt stmt;
...
static node_pt fix_expr_Indirect(node_pt e, ctxt_pt ctxt, usage_flags usage)
{
...
static node_pt fix_expr_Sizeof(node_pt e, ctxt_pt ctxt, usage_flags usage)
{
...
static typeinfo_pt arg_promotion(typeinfo_pt type)
...
static node_pt fix_undeclared_func_id(node_pt id, node_pt eArgs, scope_id_t scope)
{
    ...
    else
    {
        ...
        declarator = new_pos_node(pos, _Func_Call, id,
...
static node_pt fix_expr_Func_Call(node_pt e, ctxt_pt ctxt, usage_flags usage)
{
    ...
    if(functype)
    {
        ...
        if(!formals)
        {
            warning_at(e->node_def, "call to function with no parameter information");
        }
    }
    else
    {
    ...
    if(eArgs)
    {
        ...
        for(arg_iter = init_node_iter(&eArgs); (argp = next_list_ref(&arg_iter));)
        {
...
static node_pt fix_expr_Land(node_pt e, ctxt_pt ctxt, usage_flags usage)
{
...
static node_pt fix_expr_Lor(node_pt e, ctxt_pt ctxt, usage_flags usage)
{
    scope_id_t scope1 = new_block_scope(ctxt_scope(ctxt));
    ctxt_pt ctxt1 = new_context(scope1);
    node_pt el = fix_expr(e->node.binary.l, ctxt, USE(Is_bool));
...
static node_pt fix_expr_Trivially(node_pt e, ctxt_pt ctxt, usage_flags usage)
{
    return e;
}
...
static node_pt fix_expr_Macro_ID(node_pt e, ctxt_pt ctxt, usage_flags usage)
/* No fixup; just record unit dependency */
{
    struct macro_t* macro = e->node.macro;
    unit_dependency(current_unit(), pos_unit(macro->macro_definition),
                    !(current_unit_is_header && in_initializer));
    return e;
}

static node_pt fix_expr_Type(node_pt e, ctxt_pt ctxt, usage_flags usage)
{
    all_types_gened(e->node.typ, e->node_def);
    return e;
}

static node_pt fix_expr_Unimplemented(node_pt e, ctxt_pt ctxt, usage_flags usage)
{
    warning_at(e->node_def, "unimplemented expression type (%s) in fix_expr",
...
static fix_expr_func_t fix_expr_func(node_pt expr)
{
    switch(expr->node_kind)
    {
        case _Error:
...
node_pt fix_initializer_expr(node_pt e, typeinfo_pt t)
{
    node_pt result;
    ...
    else if(e->node_kind == _String && t->type_kind == array_of)
    {
        if(t->type_info.array.elements != -1)
        {
            result = new_pos_node(e->node_def, _Type_Cast,
                                  new_pos_node(e->node_def, _Type, t), e);
        }
        else
        {
            result = e;
...
static stmt_pt ignore_result(node_pt e, ctxt_pt ctxt)
{
    typeinfo_pt t = type_of(e);
    ...
    if(same_ada_type(t, type_int()))
        return 0;
    if(same_ada_type(t, type_string()))
        return 0;
...
static stmt_pt fix_stmt_Expr(stmt_pt stmt, ctxt_pt ctxt)
{
    node_pt expr = fix_expr(stmt->stmt.expr, ctxt, USE(Is_stmt));
    stmt_pt result = 0;

    if(expr->node_kind == _Func_Call && type_of(expr)->type_kind != void_type)
    {
...
static stmt_pt fix_stmt_SList(stmt_pt stmt, ctxt_pt ctxt)
{
    stmt_pt s;
...
static stmt_pt fix_post_return_expr(node_pt e, ctxt_pt ctxt)
/*
...
static stmt_pt fix_stmt_Return(stmt_pt stmt, ctxt_pt ctxt)
{
...
static stmt_pt fix_stmt_Switch(stmt_pt s, ctxt_pt ctxt)
{
       ...
       && stmt->stmt.compound.stmts->stmt_kind == _SList)
    {
        ...
        stmt_pt s1, s2;
        for(s1 = stmt->stmt.compound.stmts; s1; s1 = s1->stmt.stmt_list.rest)
        {
            ...
            if(s2->stmt_kind == _Case)
            {
                 ...
                 * control expression.
                 */
                s2->stmt.label.id = promote(s2->stmt.label.id, type_of(expr), ctxt);
...
static stmt_pt fix_stmt_Ifelse(stmt_pt s, ctxt_pt ctxt)
{
    node_pt expr;
    stmt_pt sIf;
    stmt_pt sElse;
...
static stmt_pt fix_stmt_For(stmt_pt s, ctxt_pt ctxt)
{
    /*
    ...
    sWhile = (set_current_scope(scope), new_stmt_While(e2->node_def, 0, e2, sBody));
    return sWhile;
}

static stmt_pt fix_stmt_Compound(stmt_pt s, ctxt_pt ctxt)
{
    /*
    ...
    for(sym = s->stmt.compound.decls, scope_id = sym ? sym->sym_scope_id : 0;
        sym; sym = sym->sym_parse_list)
    {
        /* make sure that any necessary types get generated */
        if(sym->sym_kind == var_symbol && sym->sym_type)
        {
           ...
           && !sym->is_static && sym->has_initializer && !sym->emit_initializer)
        {
            ...
            pos = expr->node_def;
            assn = new_pos_node(pos, _Assign, new_pos_node(pos, _Sym, sym), expr);
    ...
    = concat_stmts(stmts, fix_stmt_itself(s->stmt.compound.stmts));

    return s;
}

static stmt_pt fix_stmt_Labelled(stmt_pt s, ctxt_pt ctxt)
{
...
static stmt_pt fix_stmt_Default(stmt_pt s, ctxt_pt ctxt)
{
...
static stmt_pt fix_stmt_trivially(stmt_pt s, ctxt_pt ctxt)
{
    return s;
}

static stmt_pt fix_stmt_Unimplemented(stmt_pt stmt, ctxt_pt ctxt)
{
...
static fix_stmt_func_t fix_stmt_func(stmt_pt stmt)
{
    switch(stmt->stmt_kind)
    {
        case _Return:
            return fix_stmt_Return;
        case _Expr:
            return fix_stmt_Expr;
        case _SList:
            return fix_stmt_SList;
        case _Compound:
            return fix_stmt_Compound;
        case _If:
            return fix_stmt_If;
        case _While:
            return fix_stmt_While;
        case _Do:
            return fix_stmt_Do;
        case _Switch:
            return fix_stmt_Switch;
        case _Ifelse:
            return fix_stmt_Ifelse;
        case _For:
            return fix_stmt_For;

        case _Labelled:
            return fix_stmt_Labelled;
        case _Case:
            return fix_stmt_Labelled;

        case _Default:
            return fix_stmt_Default;
