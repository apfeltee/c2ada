...
static int usage(prog) char* prog;
{
    ...
    fputs("  flags:\n\n", stderr);
    show_flag("-Dname[=value]", "Define a macro with an optional value.  By default\n"
    ...
    show_flag("-exp", "Export functions, variables from .c file to Ada spec.");
    show_flag("-sih", "Suppress import declarations from included headers."
                      "(default)");
    ...
    show_flag("-rational | -vads | -gnat | -icc", "Rational | VADS | GNAT(default) | Irvine"
                                                  " as target compiler.");
