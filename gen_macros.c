...
int max;
{
    ...
    {
        fprintf(stderr, "name <%s>, ada_name <%s>, body <%s>, ", m->macro_name,
                m->macro_ada_name, m->macro_body);
...
static void gen_const_synonym(char* name, char* tname, char* value)
{
    indent_to(4);
    put_string(name);
    MAX_INDENT(max_const_name_indent);
...
static void gen_comment(macro_t* m)
{
    if(m->eol_comment)
    {
...
char* packaged_name(char* name, file_pos_t macro_pos, file_pos_t subp_pos)
{
...
int import;
{
    ...
    {
        warning(file_name(m->macro_definition), line_number(m->macro_definition),
    ...
    if(EVAL_FAILED(result) && (ada_version >= 1995) && (m->macro_body[0] == 'L')
       && (m->macro_body[1] == '"'))
    {
        is_wide_string = 1;
        ...
        if(rightparen != NULL)
        {
            ...
            if(leftparen != NULL)
            {
                ...
                if(t != NULL)
                {
                    ...
                    memset(&buf[leftparen - m->macro_body], ' ', rightparen - leftparen + 1);
                    result = cpp_eval(buf);
                    tname = type_nameof(t, 0, 0);
                    goto after_coercion;
                }
...
char* new_param_name;
{
    ...
    for(param = func_sym->sym_tags, i = 0; param != NULL; param = param->sym_parse_list, i++)
    {
        if(i == n)
        {
...
macro_t* m;
{
    ...
    if(is_pointer)
    {
        assert((func_sym->sym_type != NULL) && (func_sym->sym_type->type_kind == pointer_to)
    ...
    macro_sym->sym_ada_name = (m->macro_ada_name) ?
                              m->macro_ada_name :
    ...
    if(has_coercion)
    {
        ...
        if(coercion == NULL)
        {
            error(__FILE__, __LINE__, "type coercion (%s)", m->macro_func->mf_coercion);
    ...
    else
    {
        for(i = 0; i < m->macro_params; i++)
        {
            ...
            for(j = 0; j < m->macro_func->mf_nparams; j++)
            {
                ...
                {
                    if(is_pointer)
                        mparam = copy_nth_param(func_sym->sym_type->type_base, j, name1);
                    else
    ...
    name1 = packaged_name(func_sym->sym_ada_name, m->macro_definition, func_sym->sym_def);
    do_macro_body(body_buf, (is_function(func_sym) ? "return" : ""),
                  coercion_name(has_coercion, coercion), name1,
                  (is_pointer ? ".all" : ""), name2);
    macro_sym->has_initializer = 1;
    macro_sym->sym_value.body = new_stmt_Compound(
    m->macro_definition, NULL, new_stmt_MacroBody(m->macro_definition, body_buf));
...
void gen_macro_names()
{
    ...
    for(m = macro_list_head; m != NULL; m = m->macro_next)
    {
        if(could_be_ada_ident(m->macro_name))
        {
            ...
            else if((m->macro_body != NULL) && (m->macro_params != -1)
                    && (m->macro_func == NULL))
            {
...
int import;
{
    ...
    while(m != NULL)
    {
        if((m->macro_valid && !m->macro_gened) && (m->macro_func == NULL)
           && (m->macro_params == -1) && (t = grok_coercion(m->macro_body)) != NULL)
        {
            output_to(m->macro_declared_in_header);
            subtype_decl(m->macro_name, NULL, type_nameof(t, 0, 0), 4, NULL, m->macro_definition);
            m->macro_gened = 1;
        }
        m = m->macro_next;
    }
...
int import;
{
    ...
    while(m != NULL)
    {
        if(m->macro_valid && !m->macro_gened && (m->macro_body != NULL))
        {
            if((m->macro_func != NULL) && ((sym = find_sym(m->macro_func->mf_fname)) != NULL)
            ...
            else if((m->macro_func == NULL) && ((sym = find_sym(m->macro_body)) != NULL)
                    && (sym->sym_kind == func_symbol))
            {
                /* recognize #define func1 func2 */
...
#define COMP(str, buf) regcomp(&buf, str, 0)
...
static macro_function_t* grok_macro_function(rhs) char* rhs;
{
        ...
        return NULL;
    {
        ...
        if((first_quote != NULL) && ((first_paren == NULL) || (first_paren > first_quote)))
            return NULL;
    }
    init_regex();
    ...
    f.mf_is_pointer = 0;
    f.mf_coercion = NULL;
    if(step(rhs, ident_buf))
    { /* first identifier */
        before_func = loc1;
        after_func = loc2;
        if(step(after_func, in_parens_buf))
        { /* paren list */
...
char** argv;
{
    ...
    {
        res = grok_macro_function(" func (val1, p1, p11, 1+p1+2, p2, p22, val2)");
        if(res != NULL)
            print(res);
