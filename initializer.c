...
/* generate a 0 literal of the appropriate type */
{
    ...
    if(!predef_nul)
        predef_nul = predef_name_copy("Nul");
    ...
    switch(t->type_kind)
    {
        case pointer_to:
            ...
            break;
        case int_type:
            put_string(equal_types(t, type_char()) ? predef_nul : "0");
            break;
        case float_type:
            put_string("0.0");
            break;
        ...
        case field_type:
        default:
            put_string("{MISSING 0 INITIALIZER}");
            break;
    }
} /* gen_zero */
...
static node_pt gen_agg_item(typeinfo_pt t, node_pt e)
{
    if(e->node_kind == _List)
    {
        ...
        {
            case array_of:
                if(is_string_lit(e->node.binary.l) && equal_types(t->type_next, type_char()))
                {
...
/* Output an explicit initializer */
{
    switch(t->type_kind)
    {
        ...
        case struct_of:
        {
            ...
            for(fnext = t->type_base->sym_tags; fnext; fnext = fnext->sym_parse_list)
            {
                if(first)
                {
                    first = FALSE;
...
static node_pt* fix_agg_item(typeinfo_pt t, node_pt* ep)
{
    node_pt e = *ep;
    if(e->node_kind == _List)
    {
    ...
    switch(t->type_kind)
    {
        ...
        case array_of:
        {
            ...
            if(equal_types(tsub, type_char()))
            {
                ...
                if(e->node_kind == _Aggregate && e->node.unary->node_kind == _String)
                {
