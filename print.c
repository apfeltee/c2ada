...
char* nameof_stmt_kind(stmt_kind_t s)
{
    switch(s)
    {
        case _Labelled:
...
char* nameof_typekind(typekind_t t)
...
char* nameof_node_kind(node_kind_t n)
{
    switch(n)
    {
        case _Error:
        ...
        case _Bit_Field:
            return "_Bit_Field";
        case _Dot_Selected:
            return "_Dot_Selected";
        case _Arrow_Selected:
            return "_Arrow_Selected";
        case _Array_Index:
            return "_Array_Index";
        case _Func_Call:
            return "_Func_Call";
...
char* nameof_sym_kind(sym_kind_t s)
{
    switch(s)
    {
        case type_symbol:
...
char* nameof_cpp_eval_result_kind_t(cpp_eval_result_kind_t res)
{
    switch(res)
    {
        case eval_failed:
...
static void print_node_t(node_pt n, int indent)
{
    ...
    switch(n->node_kind)
    {
        ...
        case _Cond:
            printf("node.cond.boolval:\n");
            print_node_t(n->node.cond.boolval, indent + NEXT);
            spaces(indent);
...
static void print_typeinfo_t(typeinfo_t* t, int indent)
{
    ...
    if(print_addr("typeinfo", t, indent))
        return;
    spaces(indent);
    printf("typeinfo:\n");
    indent += NEXT;
    ...
    {
        case array_of:
            print_uns_pair("array.elements", t->type_info.array.elements, indent);
