...
#define MAKE_FLOAT(x, v)                   \
    {                                      \
        (x).eval_result_kind = eval_float; \
        (x).eval_result.fval = (v);        \
    }
#define MAKE_STRING(x, v)                   \
    {                                       \
        (x).eval_result_kind = eval_string; \
        (x).eval_result.sval = (v);         \
    }
...
static tokval_t failed(cpp_eval_state_pt s)
{
    tokval_t tmp;
    s->recover = 1;
    MAKE_FAIL(tmp);
...
static int escaped_char(int c, cpp_eval_state_pt s)
{
    int val, i;
...
static int scan_string(int c, cpp_eval_state_pt s)
{
    buffer_t buf;
...
static int scan_char_const(int c, cpp_eval_state_pt s)
{
    host_int_t cval = 0;
...
static int magnitude(int c, host_int_t* val, cpp_eval_state_pt s)
{
    host_int_t m;
    int sign;
...
static int scan_digit(int c, cpp_eval_state_pt s)
{
    host_int_t val;
    int base;
...
static int scan_id(int c, cpp_eval_state_pt s)
{
    char id[100];
    char* cp = id;
    macro_t* m;
...
static int skip_c_comment(int c, cpp_eval_state_pt s)
{
    for(;;)
    {
        if(c == BAD_INPUT)
...
int skip_cpp_comment(int c, cpp_eval_state_pt s)
{
    for(;;)
    {
        if(c == BAD_INPUT)
...
static int advance(cpp_eval_state_pt s)
{
    s->curtok = s->nexttok;
    s->curval = s->nextval;
...
static int expect(int tok, cpp_eval_state_pt s)
{
    if(s->curtok != tok)
    {
        longjmp(s->exception, 1);
...
static tokval_t cast(tokval_t val, typeinfo_pt type)
{
...
static tokval_t term(cpp_eval_state_pt s)
/* unary expression */
{
    tokval_t val;
...
static tokval_t f10(cpp_eval_state_pt s)
...
static tokval_t f9(cpp_eval_state_pt s)
/* additive expression */
{
    tokval_t l, r;
    int op;
...
static tokval_t f8(cpp_eval_state_pt s)
/* shift expression */
{
    tokval_t l, r;
    int op;
...
static tokval_t f7(cpp_eval_state_pt s)
...
static tokval_t f6(cpp_eval_state_pt s)
/* equality expression */
{
    tokval_t l, r;
    int op, tmp;
...
static tokval_t f5(cpp_eval_state_pt s)
/* and (&) expression */
{
    tokval_t l, r;
...
static tokval_t f4(cpp_eval_state_pt s)
/* xor (^) expression */
{
    tokval_t l, r;
...
static tokval_t f3(cpp_eval_state_pt s)
...
static tokval_t f2(cpp_eval_state_pt s)
...
static tokval_t f1(cpp_eval_state_pt s)
...
cpp_eval_result_t cpp_eval(char* str)
{
