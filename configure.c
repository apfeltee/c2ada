...
static source_pt file_source(file_id_t file)
{
    if(cached.file != file)
    {
        ...
        if(cached.sourceObj)
        {
            cached.declsObj = PyObject_GetAttrString(cached.sourceObj, "decl");
            assert(cached.declsObj);
            cached.macrosObj = PyObject_GetAttrString(cached.sourceObj, "macro");
        }
        else
        {
            PyErr_Clear();
...
{
    PyObject* resultObj = PyObject_CallMethod(c2ada, "source_partner", "(s)", fname);
    char* result;
...
static PyObject* stringListObj(int count, char** items)
{
    int i;
    PyObject* list;
                                   ...
                                   char* eol_comment)
{
    ...
    if(!source)
        return 0;
    macroObj = PyMapping_GetItemString(source->macrosObj, macro_name);
    if(!macroObj)
    {
        PyErr_Clear();
        return 0;
    ...
    resultObj = PyObject_CallMethod(macroObj, "rewrite", "(ssOs)", macro_name,
                                    macro_body, formalsObj, eol_comment);
